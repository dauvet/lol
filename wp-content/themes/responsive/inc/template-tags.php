...
function responsive_post_nav() {
	...
	<nav class="navigation post-navigation" role="navigation">
		...
		<div class="nav-links">
			<?php
				previous_post_link( '<div class="nav-previous">%link</div>', _x( '<span class="meta-nav">&larr;</span>&nbsp;%title', 'Previous post link', 'responsive' ) );
				next_post_link(     '<div class="nav-next">%link</div>',     _x( '%title&nbsp;<span class="meta-nav">&rarr;</span>', 'Next post link',     'responsive' ) );
			?>
		</div><!-- .nav-links -->
	</nav><!-- .navigation -->
	<?php
...
function responsive_posted_on() {
	...
	$posted_on = sprintf(
		_x( 'Posted on %s', 'post date', 'responsive' ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		_x( 'by %s', 'post author', 'responsive' ),
...
if ( ! function_exists( 'responsive_entry_footer' ) ) :
/**
 * Prints HTML with meta information for the categories, tags and comments.
 */
function responsive_entry_footer() {
	...
	if ( 'post' == get_post_type() ) {
		...
		if ( $categories_list && responsive_categorized_blog() ) {
			printf( '<span class="cat-links">' . __( 'Posted in %1$s', 'responsive' ) . '</span>', $categories_list );
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', __( ', ', 'responsive' ) );
		if ( $tags_list ) {
			printf( '<span class="tags-links">' . __( 'Tagged %1$s', 'responsive' ) . '</span>', $tags_list );
		}
	}

	if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		echo '<span class="comments-link">';
		comments_popup_link( __( 'Leave a comment', 'responsive' ), __( '1 Comment', 'responsive' ), __( '% Comments', 'responsive' ) );
		echo '</span>';
	}

	edit_post_link( __( 'Edit', 'responsive' ), '<span class="edit-link">', '</span>' );
}
endif;
...
function responsive_categorized_blog() {
	if ( false === ( $all_the_cool_cats = get_transient( 'responsive_categories' ) ) ) {
		...
		set_transient( 'responsive_categories', $all_the_cool_cats );
	}
...
function responsive_category_transient_flusher() {
	// Like, beat it. Dig?
	delete_transient( 'responsive_categories' );
}
